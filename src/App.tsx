...
export function App(): React.ReactElement {
  ...
  const [characterVariantTab, setCharacterVariantTab] = useState<'normal' | 'tainted'>(
    () => getCharacterById(scene.character.id).variant
  );
  const [activeCategory, setActiveCategory] = useState<RoomCategory>(
    () => getRoomBackdropById(scene.stageId).category
  );
  ...
  const handleSelectPreset = (preset: TemplatePreset) => {
    ...
    setScene(nextScene);
    setIsPresetDropdownOpen(false);
    setActiveCategory(getRoomBackdropById(nextScene.stageId).category);
    setCharacterVariantTab(getCharacterById(nextScene.character.id).variant);
  ...
  const handleSavePreset = (e?: React.FormEvent) => {
    ...
    const saved = saveCustomPreset(newPresetName, scene);
  ...
  const handleDeletePreset = (presetId: string, e: React.MouseEvent) => {
    ...
    setPresets(listPresets());
    if (presetToDelete && scene.presetName === presetToDelete.name) {
      setScene((prev) => ({
        ...prev,
      ...
      <header className="h-14 w-full bg-[#19161C] border-b border-[#2A252D] px-4 flex items-center justify-between shrink-0 flex-nowrap whitespace-nowrap z-30">
        ...
        <div className="flex items-center gap-3 shrink-0 whitespace-nowrap">
          ...
          <button
            ...
            onClick={() => {
              setNewPresetName(`${activeCharacterEntry.name} - ${activeRoom.name}`);
